...
if t(i)-T_last>=Ts
%    IIrE(1:3,i)=Nabila_FK(b(1:3,i));
    [H,d,G,JE,JE_dot]=Nabila_HW_9(b(:,i));
...
for i=1:length(t)-1
    ...
    if t(i)-T_last>=Ts
%         IIrE(1:3,i)=Nabila_FK(b(1:3,i));
        [H,d,G,JE,JEdot]=Nabila_HW_9(b(:,i));
...
hold on
plot(t,IIrE(2,:))
plot(t,IIrD(2,:),'r') %theta 2 des
xlabel('t (s)')
ylabel('\theta_2 (rad)')
subplot(3,1,3)
hold on
plot(t,IIrE(3,:))
plot(t,IIrD(3,:),'r') %theta 3 des
xlabel('t (s)')
ylabel('\theta_3 (rad)')